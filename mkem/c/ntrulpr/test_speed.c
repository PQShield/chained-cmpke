...
int main()
{
  ...
  print_results("mult_small: ", t, NTESTS);

  for(i=0;i<NTESTS;i++) {
    t[i] = cpucycles();
    mult_small_prefix(a, a, b);
  }
  print_results("mult_small_prefix: ", t, NTESTS);

  for(i=0;i<NTESTS;i++) {
    t[i] = cpucycles();
    mult_small_ntt(a, a,  b);
  }
  print_results("mult_small_ntt: ", t, NTESTS);
