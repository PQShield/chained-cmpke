...
int PQCLEAN_FRODOKEM640SHAKE_OPT_crypto_kem_keypair(uint8_t *pk, uint8_t *sk) {
    ...
    uint32_t S_rnd[2*PARAMS_N*PARAMS_NBAR];
    ...
    shake((uint8_t *)S_rnd, 2 * PARAMS_N * PARAMS_NBAR * sizeof(uint32_t), shake_input_seedSE, 1 + CRYPTO_BYTES);
...
int PQCLEAN_FRODOKEM640SHAKE_OPT_crypto_kem_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk) {
    ...
    uint32_t Sp_rnd[(2 * PARAMS_N + PARAMS_NBAR)*PARAMS_NBAR] = {0}; // contains secret data
    uint32_t *Ep_rnd = &Sp_rnd[PARAMS_N * PARAMS_NBAR];               // contains secret data
    uint32_t *Epp_rnd = &Sp_rnd[2 * PARAMS_N * PARAMS_NBAR];          // contains secret data
    ...
    shake((uint8_t *)Sp_rnd, (2 * PARAMS_N + PARAMS_NBAR) * PARAMS_NBAR * sizeof(uint32_t), shake_input_seedSE, 1 + CRYPTO_BYTES);
    for (size_t i = 0; i < (2 * PARAMS_N + PARAMS_NBAR) * PARAMS_NBAR; i++) {
        Sp_rnd[i] = PQCLEAN_FRODOKEM640SHAKE_OPT_LE_TO_UINT32(Sp_rnd[i]);
    }
    PQCLEAN_FRODOKEM640SHAKE_OPT_sample_n(Sp, Sp_rnd, PARAMS_N * PARAMS_NBAR);
    PQCLEAN_FRODOKEM640SHAKE_OPT_sample_n(Ep, Ep_rnd, PARAMS_N * PARAMS_NBAR);
    ...
    PQCLEAN_FRODOKEM640SHAKE_OPT_sample_n(Epp, Epp_rnd, PARAMS_NBAR * PARAMS_NBAR);
...
int PQCLEAN_FRODOKEM640SHAKE_OPT_crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk) {
    ...
    shake((uint8_t *)Sp_rnd, (2 * PARAMS_N + PARAMS_NBAR) * PARAMS_NBAR * sizeof(uint32_t), shake_input_seedSEprime, 1 + CRYPTO_BYTES);
    for (size_t i = 0; i < (2 * PARAMS_N + PARAMS_NBAR) * PARAMS_NBAR; i++) {
        Sp_rnd[i] = PQCLEAN_FRODOKEM640SHAKE_OPT_LE_TO_UINT32(Sp_rnd[i]);
    }
    PQCLEAN_FRODOKEM640SHAKE_OPT_sample_n(Sp, Sp_rnd, PARAMS_N * PARAMS_NBAR);
